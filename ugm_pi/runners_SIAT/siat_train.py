...
def getdata(datadir,dataset):
    for filename in os.listdir(datadir):
        data = loadmat(os.path.joint(datadir,filename))
        dataset=np.append(dataset,axis=1)
    print("dataset  n is {}".format(dataset.shape[0]))
    return dataset
...
class GetMRI(Dataset):
    ...
    def __getitem__(self,index):
        try:
            x=loadmat(self.data_names[index])['Img']
        except:
            print('name: ',self.data_names[index])
            print('index: ', index)
    ...
    def train(self):
        ...
        if self.config.data.dataset == 'CIFAR10':
            ...
            test_dataset = SVHN(os.path.join(self.args.run, 'datasets', 'svhn_test'), split='test', download=True,
                                transform=test_transform)
        elif self.config.data.dataset == "fastMRI":
            dataset = GetMRI(root="./train_img_8h", augment=None)
        process = 2
        dataloader = DataLoader(dataset, batch_size=self.config.training.batch_size, shuffle=True, num_workers=process)
        test_loader = DataLoader(dataset, batch_size=self.config.training.batch_size, shuffle=True,
                                 num_workers=process, drop_last=True)
    ...
    def Langevin_dynamics(self, x_mod, scorenet, n_steps=200, step_lr=0.00005):
        ...
        with torch.no_grad():
            for _ in range(n_steps):
                ...
                noise = torch.randn_like(x_mod) * np.sqrt(step_lr * 2)  #[0,1]*0.01
