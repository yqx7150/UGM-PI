...
def parse_args_and_config():
    ...
    # parse config file
    if not args.test:
        with open(os.path.join('ugm_pi/configs', args.config), 'r') as f:
            config = yaml.load(f)
    ...
    if not args.test:
        if not args.resume_training:
            if os.path.exists(args.log):
                if os.path.exists(args.log + '/checkpoint.pth'):
                    print('current write folder: {}'.format(args.log))
                    print('This floder have a checkpoint file!!')
                    assert 0
                else:
